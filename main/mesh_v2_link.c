						 ...
						 char *result, size_t result_size)
{
	...
	kheater_command_result_t command;
	if (!legacy_execute_command(text, &command)) return false;
	if (status) {
		*status = command.error == ESP_OK ? MESH_V2_CONTROL_STATUS_OK
						 : MESH_V2_CONTROL_STATUS_FAILED;
	}
	if (result && result_size > 0) {
		snprintf(result, result_size, "%s", command.result);
		result[result_size - 1] = '\0';
	}
	for (size_t i = 0; i < command.reply_count; i++) {
		/* KeeMASH UART compatibility consumes CONTROL EVENT text at node0. */
		(void)mesh_v2_node_send_event(0, command.replies[i]);
	}
	return true;
}
