...
#ifdef CONFIG_KHEATER_MESH_RELAY_ELIGIBLE
#define KHEATER_RELAY_ELIGIBLE true
#else
#define KHEATER_RELAY_ELIGIBLE false
#endif
...
{
	if (!s_rollback_pending || !heater_controller_ready()) return;
	if (tick_ms() - s_app_started_ms < ROLLBACK_HEALTH_MS) return;
...
static esp_err_t initialize_mesh_runtime(void)
{
	...
	if (!s_runtime_initialized) {
		...
		err = mesh_v2_link_init(TAG, KHEATER_RELAY_ELIGIBLE);
		if (err != ESP_OK) return err;
...
void app_main(void)
{
	esp_err_t err = heater_output_init_safe();
	if (err != ESP_OK) {
		ESP_LOGE(TAG, "safe GPIO initialization failed: %s", esp_err_to_name(err));
		abort();
	}
