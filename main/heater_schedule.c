...
static bool clock_snapshot(struct tm *local)
{
	...
	return now > (time_t)SCHEDULE_VALID_EPOCH && localtime_r(&now, local) != NULL;
}
...
static void schedule_task(void *arg)
{
	...
	for (;;) {
		...
		if (catchup) s_catchup_pending = false;
		xSemaphoreGive(s_lock);
		...
		if (catchup) {
			...
			if (latest != HEATER_SCHEDULE_NO_INDEX) {
				esp_err_t err = heater_controller_set_setpoint_runtime(
					(float)config.points[latest].target_x10 / 10.0f);
		...
		for (uint8_t i = 0; i < config.count; i++) {
			const heater_schedule_point_t *point = &config.points[i];
			if (!point_applies_on(point, weekday) || point->minute_of_day != minute) continue;
			bool execute = false;
			if (xSemaphoreTake(s_lock, portMAX_DELAY) == pdTRUE) {
				if (s_config.generation == config.generation &&
				    s_last_run_day[i] != day_key) {
					s_last_run_day[i] = day_key;
					execute = true;
				}
				xSemaphoreGive(s_lock);
			...
			if (!execute) continue;
			esp_err_t err = apply_action(point);
			if (err != ESP_OK) {
				record_error(err);
				ESP_LOGE(TAG, "point %u failed: %s", (unsigned)i,
					 esp_err_to_name(err));
			} else {
				ESP_LOGI(TAG, "point %u applied target=%.1f action=%u",
					 (unsigned)i, (double)point->target_x10 / 10.0,
